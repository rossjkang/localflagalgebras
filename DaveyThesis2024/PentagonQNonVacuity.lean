...
The second Paper-1 axiom, `phi_evalAlg_O_Q_alg_le_bound`, is covered too:
`qPhiRegular_satisfiable` exhibits a functional meeting its `QPhiRegular` gate,
so that axiom is not vacuously true either.

All four theorems are proved outright, on standard kernel axioms:
`admissibleSeq_exists` and `qPhiRegular_satisfiable` on
`[propext, Classical.choice, Quot.sound]`, `choose_eight_pos_of_guard` on
`[propext]`, and `choose_eight_eq_zero_of_lt` on none.  `AxiomCheck.lean`
pins each of those with `#guard_msgs`.
-/
...
Its statement copies the axiom's binders by hand, so a change to those binders
does not break it on its own; the `example` after it applies the axiom to the
witness and does. -/
...
-- The coupling the theorem above lacks: the axiom applied to the witness.  A
-- change to the axiom's binders fails here, whatever the theorem still says.
open PentagonQBridge in
example := pentagonQ_basis_combinatorial_identity_step1 admissibleSeq
  admissibleSeq_mono admissibleSeq_TF admissibleSeq_Reg

/-! ## The exact arithmetic fact the old form violated -/

/-- **The defect guard.**  Under `8 ≤ Δ` — the guard of the superseded first
repair, which the axiom no longer carries — the binomial in the denominator of
`genUnlabelledDensity` is nonzero, so the right-hand side of the combinatorial
identity is not identically zero.
...
/-! ## The second Paper-1 axiom: its regularity gate is satisfiable

`PentagonQBridge.phi_evalAlg_O_Q_alg_le_bound` is gated on `QPhiRegular phi`,
which asserts that `phi` arises from `pentagonQ_phi_construction` applied to a
triangle-free regular `Δ`-increasing sequence.  If no such `phi` existed the
axiom would hold vacuously and Theorem 1.2 would rest on nothing.  The sequence
above supplies one. -/

open PentagonQBridge in
/-- **Non-vacuity of the certificate-output gate.**  Some limit functional
satisfies `QPhiRegular`, so `phi_evalAlg_O_Q_alg_le_bound` is not vacuously
true. -/
theorem qPhiRegular_satisfiable :
    ∃ phi : GenLimitFunctional CG2 (GenFlagType.empty CG2)
      brrbGenGraphClass brrbGenDelta, QPhiRegular phi := by
  refine ⟨pentagonQ_phi_construction admissibleSeq admissibleSeq_mono
      admissibleSeq_TF admissibleSeq_Reg id strictMono_id, ?_⟩
  exact ⟨admissibleSeq, admissibleSeq_mono, admissibleSeq_TF, admissibleSeq_Reg,
    id, strictMono_id, rfl⟩
