...
`sum_pairsAdjOn_eq` (`PentagonDelta4.lean:803,832`): the graph side gets the
...
`edge_split` is `PentagonLocal.two_mul_prod_split` (`PentagonDelta4.lean:874`) with the
...
/-- **(B4), summed over the four letters.**  `∑_i k_i = 12`: the model's
    `PentagonLocal.sum_attach_card_eq_twelve` (`PentagonDelta4.lean:394`). -/
...
`handshake_of` is the model's `sum_pairsAdjOn_eq` (`PentagonDelta4.lean:832`) combined with
...
/-- **Steps A + B + C, model form.**  `2T = ∑_i d_{F⁺}(i)·(2k_i − 1) + 2 e₂₂`.
    The model's `PentagonLocal.two_mul_T_eq` (`PentagonDelta4.lean:934`).
...
/-- **Obligation 14, hypothesis form.**  `2T + 4n₃ + 12n₄ + ∑_i (2k_i−1)u_i = 36 + 2e₂₂`.
    Port of `PentagonLocal.shellObjective_identity` (`PentagonDelta4.lean:990`). -/
...
/-- **Obligation 15, hypothesis form.**  `e₂₂ ≤ 6`.
    Port of `PentagonLocal.e22_le_six` (`PentagonDelta4.lean:1105`).  Same two steps as the
...
/-- `2T ≤ 48`, i.e. `T ≤ 24`.  The graph-side statement is
    `PentagonLocal.shellObjective_le_24_of_regular` (`PentagonDelta4.lean:1130`). -/
