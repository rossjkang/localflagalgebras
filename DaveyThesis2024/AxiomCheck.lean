...
-- the axiom is now stated ASYMPTOTICALLY instead and carries no degree
-- hypothesis at all, a limit statement being immune to the low-degree terms.
-- These four guards are the regression: the first keeps the repaired
-- hypothesis set SATISFIABLE (so the axiom is not vacuously true), the second
-- and third pin the arithmetic fact the old form violated, from both sides of
-- degree 8, and the fourth does the same job as the first for the other
-- Paper-1 domain axiom, whose `QPhiRegular` gate would otherwise be assumed
-- rather than shown inhabited.  Guarding axiom NAMES, as the rest of this file
