//! dump the codegree-3 star root/neighbour stationarity pair for the size-8 pentagon SDP.
//!
...
fn components(n: usize) -> (V, V) {
    let s3: F = Colored::new(Graph::new(4, &[(0, 1), (1, 2), (1, 3)]), vec![0, 1, 0, 0]).into();
    let n_red: F = Colored::new(
        Graph::new(5, &[(0, 2), (0, 3), (0, 4), (1, 2), (1, 3), (1, 4)]),
        vec![0, 0, 1, 1, 1],
    )
    .into();
    let n_blk: F = Colored::new(
        Graph::new(5, &[(0, 2), (0, 3), (0, 4), (1, 2), (1, 3), (1, 4)]),
        vec![0, 1, 1, 1, 1],
    )
    .into();

    let dside = Degree::project(&s3, n).untype().no_scale();
    let nside = (Degree::project(&n_red, n).untype()
        + Degree::project(&n_blk, n).untype())
        .no_scale();
    (dside, nside)
}
