/// Bound the number of induced copies of C6 containing any particular
...
pub enum HexagonBoundGraphs {}
type F = SubClass<G, HexagonBoundGraphs>;

type N = f64;
type V = QFlag<N, F>;

impl SubFlag<G> for HexagonBoundGraphs {
    const SUBCLASS_NAME: &'static str = "Two-Coloured Hexagon Bound Graphs";
...
pub fn main() {
    ...
    let c6_path: F = Colored::new(
        Graph::new(5, &[(0, 1), (1, 2), (2, 3), (3, 4)]),
        vec![0, 1, 1, 1, 0],
    )
    .into();
    let obj = Degree::project(&c6_path, n).untype();
    ...
    let mut f = FlagSolver::new(pb, "bounded_hexagon");
    f.init();
    f.print_report();

    let result = -f.optimal_value.expect("Failed to get optimal value");

    println!("Optimal value (lambda6): {}", result);
    println!("=> rho_C6 <= lambda6/12 = {}", result / 12.0);
}
