    ...
    let cholesky = match cs.0 {
        Invariant => {
            let inv_mat = crate::density::class_matrices(&cs.1.invariant_classes().get())
                .0
                .map(|&x| x as f64);
            let cho_t: Array2<f64> = cholesky0.t().to_owned();
            let rows = inv_mat.rows();
            let cols = cho_t.ncols();
            let mut result: Array2<f64> = Array2::zeros((rows, cols));
            for (val, (r, c)) in inv_mat.iter() {
                let v = *val;
                for k in 0..cols {
                    result[[r, k]] += v * cho_t[[c, k]];
                }
            }
            result.t().to_owned()
        }
    ...
    let eigenvectors = match cs.0 {
        Invariant => {
            let inv_mat = crate::density::class_matrices(&cs.1.invariant_classes().get())
                .0
                .map(|&x| x as f64);
            let ev_t: Array2<f64> = eigenvectors.t().to_owned();
            // Manual sparse-dense product to avoid sprs/ndarray version skew.
            let rows = inv_mat.rows();
            let cols = ev_t.ncols();
            let mut result: Array2<f64> = Array2::zeros((rows, cols));
            for (val, (r, c)) in inv_mat.iter() {
